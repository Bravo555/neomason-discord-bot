...
type SimpleResponse = (String, String, u64);
    ...
    fn message(&self, ctx: Context, msg: Message) {
...

        {
            ...
            for (keyword, response, guildid) in simple_responses {
                if message.contains(keyword) && msg.guild_id.unwrap().as_u64() == guildid {
                    ...
                    .send_message(&ctx, |f| {
                        f.embed(|e| {
                            let description = responses
                                ...
                                .map(|(key, resp, _)| format!("{} => {}", key, resp))
                                .collect::<Vec<String>>()
                                .join("\n");
    ...
    if body.starts_with("\"") {
        ...
        let keyword = keyword.unwrap();
        {
            ...
            if responses.iter().any(|(key, _r, current_guildid)| {
                key == keyword && guildid.as_u64() == current_guildid
            }) {
...
fn add_response(ctx: &Context, keyword: &str, response: &str, guildid: GuildId) {
    ...
    responses.push((keyword.to_string(), response.to_string(), guildid.into()));
}
